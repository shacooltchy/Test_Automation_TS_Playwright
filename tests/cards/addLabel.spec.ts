...
test.describe('Add card label tests', {tag: '@cards'}, () => {
    ...
    test('Add a label to a card', async( {boardDetailsPage} ) => {
        ...
        await test.step('Click the Labels button in the Add to card dialog', async() => {
            await boardDetailsPage.cardEditor.addToCardDialog.clickAddLabel();
        });
        ...
        await test.step('Verify label is added to the card', async() => {
