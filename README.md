...
1. Project setup
    1. Ensure you have Playwright and at least chromium installed - 'npm install @playwright/test' --> 'npx playwright install chromium'
    2. Add .env file based on .env.example

2. Running the tests from the teminal
    1. Run 'npx playwright test --config=./config/playwright.config.ts --project setup', this command will run 'authenticate' setup which will login and save storage state. There is no need to run it before every test run, but it is valid for a limited time. It might be needed to run this test in headed mode to manually workaround the mfa.
